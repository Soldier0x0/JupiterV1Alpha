...
def validate_request_size(request_size: int, max_size: int = 10485760) -> tuple[bool, Optional[str]]:
    """Validate request size and return (is_valid, error_msg)."""
    if request_size <= max_size:
        return True, None
    else:
        return False, f"Request too large: {request_size} bytes (max {max_size} bytes)"
